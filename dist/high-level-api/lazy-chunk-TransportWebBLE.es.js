...
import { c as DisconnectedDevice, b as TransportError, T as Transport, a as TransportOpenUserCancelled } from './lazy-chunk-Transport.es.js';
...
const sendAPDU = (write, apdu, mtuSize) => {
    const chunks = chunkBuffer(apdu, i => mtuSize - (i === 0 ? 5 : 3)).map((buffer$1, i) => {
        const head = buffer.Buffer.alloc(i === 0 ? 5 : 3);
        head.writeUInt8(TagId$1, 0);
        head.writeUInt16BE(i, 1);
        if (i === 0) {
            head.writeUInt16BE(apdu.length, 3);
        }
        return buffer.Buffer.concat([head, buffer$1]);
    });
...
const receiveAPDU = (rawStream) => new Observable(o => {
    let notifiedIndex = 0;
    let notifiedDataLength = 0;
    let notifiedData = buffer.Buffer.alloc(0);
    const sub = rawStream.subscribe({
        ...
        next: value => {
            ...
            notifiedIndex++;
            notifiedData = buffer.Buffer.concat([notifiedData, data]);
...
const monitorCharacteristic = (characteristic) => Observable.create(o => {
    ...
    function onCharacteristicValueChanged(event) {
        ...
        if (characteristic.value) {
            o.next(buffer.Buffer.from(characteristic.value.buffer));
        }
    }
    ...
    inferMTU() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield this.exchangeAtomicImpl(() => __awaiter(this, void 0, void 0, function* () {
                try {
                    mtu =
                        (yield firstValueFrom(merge(this.notifyObservable.pipe(first(buffer => buffer.readUInt8(0) === 0x08), map(buffer => buffer.readUInt8(5))), defer(() => from(this.write(buffer.Buffer.from([0x08, 0, 0, 0, 0])))).pipe(ignoreElements())))) + 3;
                }
                catch (e) {
