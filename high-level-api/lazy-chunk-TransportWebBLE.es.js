...
import { g as libEs, d as dist, l as log, T as Transport, c as TransportOpenUserCancelled, b as DisconnectedDevice } from './lazy-chunk-index.es.js';
import { c as createCommonjsModule, u as unwrapExports } from './lazy-chunk-_commonjsHelpers.es.js';
import { g as getBluetoothServiceUuids, a as getInfosForServiceUuid } from './lazy-chunk-index.es2.js';
...
const sendAPDU = (write, apdu, mtuSize) => {
  ...
  return _esm5.Observable.create(o => {
    ...
    main().then(() => {
      ...
      terminated = true;
      (0, libEs.log)("ble-error", "sendAPDU failure " + String(e));
      o.error(e);
    });

    const unsubscribe = () => {
      if (!terminated) {
        (0, libEs.log)("ble-verbose", "sendAPDU interruption");
        terminated = true;
      }
    };
...
const receiveAPDU = rawStream => _esm5.Observable.create(o => {
  ...
  const sub = rawStream.subscribe({
    complete: () => {
      o.error(new dist.DisconnectedDevice());
      sub.unsubscribe();
    },
    error: e => {
      (0, libEs.log)("ble-error", "in receiveAPDU " + String(e));
      o.error(e);
      sub.unsubscribe();
    },
    next: value => {
      ...
      if (tag !== TagId) {
        o.error(new dist.TransportError("Invalid tag " + tag.toString(16), "InvalidTag"));
        return;
      }

      if (notifiedIndex !== index) {
        o.error(new dist.TransportError("BLE: Invalid sequence number. discontinued chunk. Received " + index + " but expected " + notifiedIndex, "InvalidSequence"));
        return;
      }
      ...
      if (notifiedData.length > notifiedDataLength) {
        o.error(new dist.TransportError("BLE: received too much data. discontinued chunk. Received " + notifiedData.length + " but expected " + notifiedDataLength, "BLETooMuchData"));
        return;
      }
  ...
  constructor(device, writeCharacteristic, notifyObservable, deviceModel) {
    ...
    this.exchange = apdu => this.exchangeAtomicImpl(async () => {
      try {
        ...
        log("apdu", `=> ${msgIn}`);
        const data = await merge(this.notifyObservable.pipe(receiveAPDU_2), sendAPDU_2(this.write, apdu, this.mtuSize)).toPromise();
