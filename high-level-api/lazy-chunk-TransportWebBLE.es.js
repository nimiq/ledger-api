...
const sendAPDU = (write, apdu, mtuSize) => {
  ...
  return _esm5.Observable.create(o => {
    ...
    main().then(() => {
      ...
      terminated = true;
      (0, lib.log)("ble-error", "sendAPDU failure " + String(e));
      o.error(e);
    });

    const unsubscribe = () => {
      if (!terminated) {
        (0, lib.log)("ble-verbose", "sendAPDU interruption");
        terminated = true;
      }
    };
...
const receiveAPDU = rawStream => _esm5.Observable.create(o => {
  let notifiedIndex = 0;
  ...
  const sub = rawStream.subscribe({
    ...
    error: e => {
      (0, lib.log)("ble-error", "in receiveAPDU " + String(e));
      o.error(e);
      sub.unsubscribe();
    },
    next: value => {
...
const monitorCharacteristic = characteristic => _esm5.Observable.create(o => {
  (0, lib.log)("ble-verbose", "start monitor " + characteristic.uuid);
  ...
  return () => {
    (0, lib.log)("ble-verbose", "end monitor " + characteristic.uuid);
...
const availability = () => _esm5.Observable.create(observer => {
...
async function open(deviceOrId, needsReconnect) {
  ...
  if (typeof deviceOrId === "string") {
    if (transportsCache[deviceOrId]) {
      (0, lib.log)("ble-verbose", "Transport in cache, using that.");
  ...
  if (!device.gatt.connected) {
    (0, lib.log)("ble-verbose", "not connected. connecting...");
  ...
  const notifyObservable = (0, monitorCharacteristic_1.monitorCharacteristic)(notifyC).pipe((0, operators.tap)(value => {
    (0, lib.log)("ble-frame", "<= " + value.toString("hex"));
  }), (0, operators.share)());
  ...
  const onDisconnect = e => {
    ...
    (0, lib.log)("ble-verbose", `BleTransport(${transport.id}) disconnected`);
...
class BluetoothTransport extends _hwTransport.default {
  ...
  constructor(device, writeCharacteristic, notifyObservable, deviceModel) {
    ...
    this.exchange = apdu => this.exchangeAtomicImpl(async () => {
      try {
        ...
        (0, lib.log)("apdu", `=> ${msgIn}`);
        const data = await (0, _esm5.merge)(this.notifyObservable.pipe(receiveAPDU_1.receiveAPDU), (0, sendAPDU_1.sendAPDU)(this.write, apdu, this.mtuSize)).toPromise();
        const msgOut = data.toString("hex");
        (0, lib.log)("apdu", `<= ${msgOut}`);
        return data;
      } catch (e) {
        (0, lib.log)("ble-error", "exchange got " + String(e));
    ...
    this.write = async buffer => {
      (0, lib.log)("ble-frame", "=> " + buffer.toString("hex"));
    ...
    (0, lib.log)("ble-verbose", `BleTransport(${String(this.id)}) new instance`);
  }
  ...
  async inferMTU() {
    ...
    await this.exchangeAtomicImpl(async () => {
      try {
        mtu = (await (0, _esm5.merge)(this.notifyObservable.pipe((0, operators.first)(buffer => buffer.readUInt8(0) === 0x08), (0, operators.map)(buffer => buffer.readUInt8(5))), (0, _esm5.defer)(() => (0, _esm5.from)(this.write(Buffer.from([0x08, 0, 0, 0, 0])))).pipe((0, operators.ignoreElements)())).toPromise()) + 3;
      } catch (e) {
        (0, lib.log)("ble-error", "inferMTU got " + String(e));
        this.device.gatt.disconnect();
        throw e;
      }
    });
    ...
    if (mtu > 23) {
      const mtuSize = mtu - 3;
      (0, lib.log)("ble-verbose", `BleTransport(${String(this.id)}) mtu set to ${String(mtuSize)}`);
...
BluetoothTransport.disconnect = async id => {
  (0, lib.log)("ble-verbose", `user disconnect(${id})`);
...
var TransportWebBLE$1 = unwrapExports(TransportWebBLE);

export default TransportWebBLE$1;
export { TransportWebBLE as __moduleExports };
