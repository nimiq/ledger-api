...
export default abstract class RequestNimiq<T> extends Request<T> {
    ...
    protected constructor(type: RequestTypeNimiq, expectedWalletId?: string) {
        ...
        // Ignore errors.
        Promise.all([
            RequestNimiq._loadLowLevelApi(), // needed by all requests
            this._isWalletIdDerivationRequired ? RequestNimiq._loadNimiq() : null,
        ]).catch(() => {});
    }
    ...
    public async checkCoinAppConnection(transport: Transport): Promise<CoinAppConnection> {
        ...
        // with WebUsb being used by default, we ignore this side effect for !_isWalletIdDerivationRequired case.
